...
Entonces el sistema actualiza y muestra la medalla correspondiente automáticamente.
    ...
    <tr><td>TS-001</td><td>Endpoint para registro</td><td>Como developer, quiero crear un endpoint para permitir a los usuarios registrarse en la aplicación para que puedan autenticarse posteriormente y acceder a sus cuentas.</td><td>Escenario 1: Dado que el Developer implementa correctamente el endpoint de registro, cuando un usuario envía una solicitud POST con un nombre, correo y contraseña válidos, entonces la respuesta debe ser 201 Created y el usuario debe ser guardado en la base de datos.<br><br>Escenario 2: Dado que el Developer valida unicidad del correo electrónico, cuando un usuario intenta registrarse con un correo ya registrado, entonces la respuesta debe ser 409 Conflict.<br><br>Escenario 3: Dado que el Developer valida el formato de entrada, cuando un usuario envía datos inválidos, entonces la respuesta debe ser 400 Bad Request.<br><br>Escenario 4: Dado que el Developer protege la contraseña, cuando se almacena un usuario, entonces la contraseña debe ser cifrada.</td><td>EP-001</td></tr>
    <tr><td>TS-002</td><td>Endpoint para inicio de sesión</td><td>Como developer, quiero crear un endpoint para permitir a los usuarios iniciar sesión en la aplicación para que obtengan un token de autenticación y accedan a funcionalidades protegidas.</td><td>Escenario 1: Dado que el Developer implementa el login, cuando un usuario envía correo y contraseña válidos, entonces recibe un token de autenticación.<br><br>Escenario 2: Dado que el correo no está registrado, cuando se hace la solicitud, entonces la respuesta es 401 Unauthorized.<br><br>Escenario 3: Dado que la contraseña es incorrecta, cuando se hace la solicitud, entonces la respuesta es 401 Unauthorized.<br><br>Escenario 4: Dado que faltan datos en la solicitud, cuando se hace la petición, entonces la respuesta es 400 Bad Request.</td><td>EP-001</td></tr>
    <tr><td>TS-003</td><td>Endpoint para obtener usuarios de un grupo</td><td>Como developer, quiero crear un endpoint para obtener los usuarios de un grupo para visualizar su información desde el frontend del equipo.</td><td>Escenario 1: Dado que el Developer implementa el endpoint, cuando se envía un groupId válido, entonces se devuelve la lista de usuarios.<br><br>Escenario 2: Dado que el groupId es inválido, cuando se hace la solicitud, entonces la respuesta es 404 Not Found.</td><td>EP-001</td></tr>
    <tr><td>TS-004</td><td>Endpoint para crear un grupo</td><td>Como developer, quiero crear un endpoint para registrar nuevos grupos en la base de datos para que los usuarios puedan organizarse y trabajar colaborativamente.</td><td>Escenario 1: Dado que se envía un nombre y descripción válidos, cuando se hace la solicitud, entonces se crea el grupo y se devuelve 201 Created.<br><br>Escenario 2: Dado que faltan datos o hay errores de formato, cuando se hace la solicitud, entonces se responde 400 Bad Request.</td><td>EP-001</td></tr>
    <tr><td>TS-005</td><td>Endpoint para crear un reto</td><td>Como developer, quiero crear un endpoint para registrar nuevos retos personalizados para que puedan ser asignados a los grupos.</td><td>Escenario 1: Dado que se envían todos los campos requeridos, cuando se hace la solicitud, entonces se crea el reto con 201 Created.<br><br>Escenario 2: Dado que faltan datos o hay errores, cuando se hace la solicitud, entonces se responde 400 Bad Request.</td><td>EP-003</td></tr>
    <tr><td>TS-006</td><td>Endpoint para listar retos por grupo</td><td>Como developer, quiero crear un endpoint para obtener los retos asignados a un grupo para que los miembros puedan visualizar sus tareas.</td><td>Escenario 1: Dado que se envía un groupId válido, cuando se hace la solicitud, entonces se devuelve una lista de retos con 200 OK.<br><br>Escenario 2: Dado que el grupo no tiene retos, cuando se hace la solicitud, entonces se devuelve una lista vacía.</td><td>EP-003</td></tr>
    <tr><td>TS-007</td><td>Endpoint para enviar respuesta a un reto</td><td>Como developer, quiero crear un endpoint para permitir a los estudiantes enviar sus respuestas a los retos para registrar su progreso.</td><td>Escenario 1: Dado que se envía una solicitud con datos válidos, cuando se hace el POST, entonces se guarda la entrega con 201 Created.<br><br>Escenario 2: Dado que el reto no existe, cuando se intenta entregar, entonces se responde 404 Not Found.<br><br>Escenario 3: Dado que el formato es incorrecto, cuando se hace la solicitud, entonces se responde 400 Bad Request.</td><td>EP-003</td></tr>
    <tr><td>TS-008</td><td>Endpoint para agregar o eliminar integrantes de un grupo</td><td>Como developer, quiero permitir agregar o eliminar miembros de un grupo para mantener actualizada su composición.</td><td>Escenario 1: Dado que se envía un userId válido, cuando se hace POST, entonces se agrega el usuario.<br><br>Escenario 2: Dado que se hace DELETE con un userId válido, cuando se procesa la solicitud, entonces se elimina del grupo.<br><br>Escenario 3: Dado que el userId no existe, cuando se hace la solicitud, entonces se responde 404 Not Found.</td><td>EP-001</td></tr>
    <tr><td>TS-009</td><td>Endpoint para generar código de invitación</td><td>Como developer, quiero generar un código único de invitación para grupos para que los usuarios puedan unirse fácilmente.</td><td>Escenario 1: Dado que el groupId es válido, cuando se hace la solicitud, entonces se genera un código y se devuelve.<br><br>Escenario 2: Dado que el grupo no existe, cuando se hace la solicitud, entonces se responde 404 Not Found.</td><td>EP-001</td></tr>
    <tr><td>TS-010</td><td>Endpoint para validar código de invitación</td><td>Como developer, quiero validar un código de grupo enviado por un usuario para confirmar su validez antes de permitirle unirse.</td><td>Escenario 1: Dado que el código es válido, cuando se hace la solicitud, entonces se devuelve información del grupo.<br><br>Escenario 2: Dado que el código no es válido, cuando se hace la solicitud, entonces se responde 400 Bad Request.</td><td>EP-001</td></tr>
    <tr><td>TS-011</td><td>Endpoint para actualizar fecha límite de reto</td><td>Como developer, quiero actualizar la fecha límite de un reto existente para permitir mayor flexibilidad a los equipos durante su desarrollo.</td><td>Escenario 1: Dado que se envía una fecha válida, cuando se hace la solicitud, entonces se actualiza la fecha del reto.<br><br>Escenario 2: Dado que la fecha es inválida, cuando se hace la solicitud, entonces se responde 400 Bad Request.<br><br>Escenario 3: Dado que el reto no existe, cuando se hace la solicitud, entonces se responde 404 Not Found.</td><td>EP-003</td></tr>
  </tbody>
</table>
